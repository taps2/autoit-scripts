...
HotKeySet("!x", "enable")
	...
	while 1
		if $start = true then
			...
			$temp = TCPReceive($ConnectedSocket)
			If @Error = 0 Then
					if StringInStr($temp, "target1") <> 0 Then
						_SetTarget1()
					endif
					if StringInStr($temp, "target2") <> 0 Then
						_SetTarget2()
					endif
					if StringInStr($temp, "target3") <> 0 Then
						_SetTarget3()
					endif
					if StringInStr($temp, "target4") <> 0 Then
						_SetTarget4()
					endif
					if StringInStr($temp, "syncdrop") <> 0 Then
						_SyncDrop()
					endif
					If _IsPressedEx('31') Then _SetTarget1()
					If _IsPressedEx('32') Then _SetTarget2()
					If _IsPressedEx('33') Then _SetTarget3()
					If _IsPressedEx('34') Then _SetTarget4()
					If _IsPressedEx('1b') Then _Exit()
					If _IsPressedEx('35') Then _SyncDrop()
			endif
		endif
	wend
